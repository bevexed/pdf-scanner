...
- [ ] **Step 1: 写失败测试**(8 个,含摘要页跳过 + VAT 陷阱 + 混排页)
...
def _invoice_no(doc):
    """账单号取自首页 'XXXXX0612' 之类:第0页第一个非纯数字客户账号 token。简化:取首页首个大写字母数字混合词。"""
    if len(doc) == 0:
        return ""
...
def parse_pdf(pdf_path, page_range=None, progress_cb=None):
    ...
    for pno in range(lo, hi):
        ...
        if _is_detail_page(page):
            out.extend(_parse_detail_page(page, pno, invoice_no))
        if progress_cb:
...
Run: `./venv/bin/pytest tests/test_pdf_parser.py -v`
Expected: PASS (8 passed)。若 sender/amount 断言失败,打印实际 mask_rects 对照阈值微调 config。
...
def lookup(db, awb):
    """返回该 AWB 在已提交批次里的所有记录(候选列表),按明细优先+页码排序。"""
    with _conn(db) as c:
        rows = c.execute(
    ...
    out = []
    for r in rows:
        out.append({"awb": r["awb"], "pdf_hash": r["pdf_hash"], "invoice_no": r["invoice_no"],
...
def test_query_returns_candidates(monkeypatch):
    recs = [
        {"awb": "889635416339", "invoice_no": "INV_A", "source_page_index": 56,
         "record_type": "detail", "pdf_hash": "h", "rect": [0,160,595,285],
         "mask_rects": [], "has_extra": True, "has_duty": False},
        {"awb": "889635416339", "invoice_no": "INV_B", "source_page_index": 60,
    ...
    monkeypatch.setattr(appmod.exporter, "export_masked",
                        lambda *a, **k: "data/exports/889635416339_INV_A.png")
...
def test_query_incomplete_mask(monkeypatch):
    rec = [{"awb": "889635416339", "invoice_no": "INV_A", "source_page_index": 56,
            "record_type": "detail", "pdf_hash": "h", "rect": [0,160,595,285],
            "mask_rects": [], "has_extra": True, "has_duty": False}]
...
def query():
    ...
    for awb in awbs:
        ...
        for rec in recs:   # 候选列表:每条一行
            ...
            out = os.path.join(config.EXPORT_DIR, f"{awb}_{rec['invoice_no']}.png")
            try:
...
Expected: 全部 PASS(config5 + fees2 + classifier6 + parser8 + indexer5 + exporter3 + app5 = 34)
...
10. 测试数量 → 已重算标注(classifier6/parser8 等),Task9 汇总 34 ✅
11. 页码 0 基 → 头部+Task4 注明,parse_pdf 用 0 基 page_range ✅

**Type 一致性**:ticket dict(awb/invoice_no/source_page_index/record_type/rect/fee_rows/mask_rects/has_extra/has_duty)贯穿 parser→indexer.insert→lookup→app.query;fee_rows 元素 `{label,amount_text,amount_rect}` 在 fees→classifier→indexer 一致;mask_rects 元素 `{kind,rect}` 在 parser→exporter 一致;IncompleteMaskError 在 exporter 定义、app 捕获。✅

**Placeholder**:无 TBD/TODO,代码完整。`_invoice_no` 用首页大写字母+数字词(如 XXXXX0612),已实测该 PDF 首页含此格式。
