...
> 注:`threading` 已在 Task 3 顶层 import;`_use_tray()` 在 Task 4b 定义(本 Task 先加一个恒为 True 的临时桩,Task 4b 再替换)。**临时桩必须放在 `main()` 定义之前(或至少在 `if __name__ == "__main__"` 之前)**,否则直接运行 `desktop.py` 时 `main()` 调用 `_use_tray()` 会 `NameError`。临时桩:
> ```python
> def _use_tray():
>     return True
> ```
...
- 自动化:`pytest` 全绿(`is_busy()` 3 例、`FlaskServer`/`wait_until_ready` 5 例含未 start 安全 close 与 shutdown 幂等、原有用例)。
